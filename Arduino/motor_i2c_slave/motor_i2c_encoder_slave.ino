...
volatile uint8_t motorMode = 0x00;
volatile unsigned long lastI2CReceiveTime = 0;
...
void loop() {
  ...
  if (now - lastRPMCalcTime >= interval) {
    lastRPMCalcTime = now;

    rpm_B = ((double)B_wheel_pulse_count / shaft_ppr) * 60000 / interval;
    rpm_A = ((double)A_wheel_pulse_count / shaft_ppr) * 60000 / interval;
    B_wheel_pulse_count = 0;
    A_wheel_pulse_count = 0;

    Serial.print("RPM_A: "); Serial.print(rpm_A);
    Serial.print("   RPM_B: "); Serial.println(rpm_B);
  }
...
void receiveData(int byteCount) {
  ...
  uint8_t discard = Wire.read();           // Ignore register byte
  motorMode = Wire.read();                 // 0x44 or 0x55
