...
{
    Wire.beginTransmission((uint8_t)AK09918_I2C_ADDR); // Explicit cast to uint8_t
    Wire.write((uint8_t)reg);                          // Explicit cast to uint8_t
    Wire.endTransmission();

    Wire.requestFrom((uint8_t)AK09918_I2C_ADDR, (uint8_t)8); // Explicit cast to uint8_t
    for (int i = 0; i < 8; i++)
    {
        buf[i] = Wire.read();
    }
    return 0; // Add a return statement to avoid warnings
}
